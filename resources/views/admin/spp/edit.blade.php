<div class="block space-y-4 md:flex md:space-y-0 md:space-x-4">
    ...
    <form action="/admin/spp/{{ $item->id }}/edit/" class="badge bg-warning" method="POST">
        @method('edit')
        @csrf
    <button data-modal-target="edit-spp{{ $item->id }}" id="button-edit" data-modal-toggle="edit-spp{{ $item->id }}" class="px-2 py-2" type="button">
...
<!-- Main Modal -->
<div id="edit-spp{{ $item->id }}" tabindex="-1" data-modal-backdrop="static" data-te-keyboard="false" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-5 border-b rounded-t dark:border-gray-600">
                ...
                <button type="button" onclick="reset()" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="edit-spp{{ $item->id }}">
            ...
            <div class="p-6 space-y-6">
                <form action="/admin/spp/{{ $item->id }}" id="edit" method="POST">
                    @method('put')
                    @csrf
                    <div class="grid gap-6 mb-6 md:grid-rows-2">
                        <div>
                            <label for="tahun" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tahun</label>
                            <input type="number" name="tahun" id="tahun" value="{{$item->tahun}}"
                        ...
                        <div>
                            ...
                            <input type="number" name="nominal" id="nominal" value="{{ $item->nominal }}"
